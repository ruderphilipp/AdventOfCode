package com.adventofcode.yr2016;

import org.github.ruderphilipp.FileHelper;
import org.junit.jupiter.api.Test;

import java.awt.*;
import java.util.Arrays;
import java.util.List;

import static org.assertj.core.api.Assertions.assertThat;

/**
 * You're airdropped near Easter Bunny Headquarters in a city somewhere. "Near", unfortunately, is as close as you can
 * get - the instructions on the Easter Bunny Recruiting Document the Elves intercepted start here, and nobody had time
 * to work them out further.
 * <p>
 * The Document indicates that you should start at the given coordinates (where you just landed) and face North. Then,
 * follow the provided sequence: either turn left (L) or right (R) 90 degrees, then walk forward the given number of
 * blocks, ending at a new intersection.
 * <p>
 * There's no time to follow such ridiculous instructions on foot, though, so you take a moment and work out the
 * destination. Given that you can only walk on the street grid of the city, how far is the shortest path to the
 * destination?
 */
public class Day01Test {

    /**
     * Following R2, L3 leaves you 2 blocks East and 3 blocks North, or 5 blocks away.
     */
    @Test
    void example1_1() {
        // given
        var input = List.of(
                "R2",
                "L3"
        );
        var expectedCoordinate = new Point(2, 3);
        var expectedResult = 5;

        // when
        var path = new Day01_2016(input);
        var resultCoordinate = path.getCoordinate();
        var result = path.getDistance();

        // then
        assertThat(resultCoordinate).isEqualTo(expectedCoordinate);
        assertThat(result).isEqualTo(expectedResult);
    }

    /**
     * R2, R2, R2 leaves you 2 blocks due South of your starting position, which is 2 blocks away.
     */
    @Test
    void example1_2() {
        // given
        var input = List.of(
                "R2",
                "R2",
                "R2"
        );
        var expectedCoordinate = new Point(0, -2);
        var expectedResult = 2;

        // when
        var path = new Day01_2016(input);
        var resultCoordinate = path.getCoordinate();
        var result = path.getDistance();

        // then
        assertThat(resultCoordinate).isEqualTo(expectedCoordinate);
        assertThat(result).isEqualTo(expectedResult);
    }

    /**
     * R5, L5, R5, R3 leaves you 12 blocks away.
     */
    @Test
    void example1_3() {
        // given
        var input = List.of(
                "R5",
                "L5",
                "R5",
                "R3"
        );
        var expectedCoordinate = new Point(10, 2);
        var expectedResult = 12;

        // when
        var path = new Day01_2016(input);
        var resultCoordinate = path.getCoordinate();
        var result = path.getDistance();

        // then
        assertThat(resultCoordinate).isEqualTo(expectedCoordinate);
        assertThat(result).isEqualTo(expectedResult);
    }

    /**
     * How many blocks away is Easter Bunny HQ?
     */
    @Test
    void riddle01() {
        // given
        var input = FileHelper.getFileContent("2016_01.txt");
        // only line one contains instructions as CSV
        var values = input.get(0).split(", ");

        // when
        var path = new Day01_2016(Arrays.asList(values));
        var resultCoordinate = path.getCoordinate();
        var result = path.getDistance();

        // then
        assertThat(resultCoordinate).isEqualTo(new Point(164, -136));
        assertThat(result).isEqualTo(300);
    }

    /**
     * Then, you notice the instructions continue on the back of the Recruiting Document. Easter Bunny HQ is actually
     * at the first location you visit twice.
     * <p>
     * For example, if your instructions are R8, R4, R4, R8, the first location you visit twice is 4 blocks away, due
     * East.
     */
    @Test
    void example2_1() {
        // given
        var input = List.of(
                "R8", "R4", "R4", "R8"
        );

        // when
        var path = new Day01_2016_withPathCollision(input);
        var resultCoordinate = path.getCoordinate();
        var result = path.getDistance();

        // then
        assertThat(resultCoordinate).isEqualTo(new Point(4, 0));
        assertThat(result).isEqualTo(4);
    }

    /**
     * How many blocks away is the first location you visit twice?
     */
    @Test
    void riddle02() {
        // given
        var input = FileHelper.getFileContent("2016_01.txt");
        // only line one contains instructions as CSV
        var values = input.get(0).split(", ");

        // when
        var path = new Day01_2016_withPathCollision(Arrays.asList(values));
        var resultCoordinate = path.getCoordinate();
        var result = path.getDistance();

        // then
        assertThat(resultCoordinate).isEqualTo(new Point(9, -150));
        assertThat(result).isEqualTo(159);
    }
}
